...
class ImprovedTradingEnv(gym.Env):
    # Enhanced trading environment with long/short only, no hold, maybe for an update
    # Proper transaction cost calculation based on trade size, not net worth
    
    # Transaction costs are calculated as:
    #   cost per trade = amount_traded x fee_rate
    #   when flipping position: 2 trades = 2 x fee
    #   amount traded = net_worth since we are always fully invested
    ...
    def __init__(self, df, initial_balance=1000, transaction_cost=0.0001, lookback_window=20):
        ...
        self.transaction_cost = transaction_cost  # 0.0002 = 0.02%
        ...
        # Actions: 0 = Long, 1 = Short no hold
    ...
    def _calculate_reward(self, position_return_before_costs, position_changed, trade_cost_pct):
        ...
        # position_return_before_costs is the return from price movement
        # trade_cost_pct is the transaction cost as percentage of net worth
        ...
        # Optional: penalize excessive trading to have less noise
    ...
    def step(self, action):
        ...
        # Calculate return based on position we held during this period
        # not the new position we are taking
        ...
        # Net worth must also be updated with returns, then costs are deducted
        ...
        if position_changed:
            ...
            # When we flip position long to short or short to long->
            #   Close current position: trade net_worth, pay fee
            #   Open new position: trade net_worth, pay fee
        ...
        self.returns_after_costs.append(current_return)  # includes cost impact
