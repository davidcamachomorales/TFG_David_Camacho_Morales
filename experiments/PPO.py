# To know how to execute it go to README.md
...
def train_and_evaluate_single_seed(
    ...
    # Train and evaluate PPO for a single seed
...
# this one is a smaller test with ablation but always with baseliners in combinations
...
for idx, scenario_id in enumerate(list_scenario_id):
    ...
    try:
        ...
        with open(f"{save_dir_json}/{scenario_id}_PPO_results.json", 'w') as f:
...
# Save summary
df_batch_summary.to_csv(f'{PROJECT_ROOT}/results/PPO_batch_summary.csv', index=False)
# Save all seed results 
df_all_seeds = pd.DataFrame(all_seed_results)
df_all_seeds.to_csv(f'{PROJECT_ROOT}/results/PPO_all_seed_results.csv', index=False)